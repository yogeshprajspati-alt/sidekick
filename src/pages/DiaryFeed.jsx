...
export default function DiaryFeed() {
    ...
    const slideDirection = tabIndex >= prevTabIndexRef.current ? 1 : -1
    ...
    return (
        <div className="relative h-full flex flex-col overflow-hidden">
            ...
            <header className="relative z-20 flex-shrink-0">
                    ...
                    {isMenuOpen && (
                        <>
                            ...
                            <motion.div
                                ...
                                style={{ background: 'rgba(6,4,6,0.75)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}
                            ...
                            <motion.div
                                ...
                                style={{
                                    width: 272, maxWidth: '82vw',
                                    background: 'linear-gradient(160deg, rgba(22,16,18,0.99) 0%, rgba(12,9,10,0.99) 100%)',
                            ...
                            >
                                ...
                                <div style={{
                                    ...
                                    background: 'linear-gradient(180deg, transparent 0%, rgba(183,110,121,0.5) 35%, rgba(212,175,55,0.35) 65%, transparent 100%)',
                                }} />
                                ...
                                <div className="px-5 pt-5 pb-4" style={{ position: 'relative', zIndex: 1 }}>
                                    <div className="flex items-center justify-between mb-5">
                                        <h2 className="font-script text-2xl text-rose-gold-light">Menu</h2>
                                        <motion.button
                                            onClick={() => setIsMenuOpen(false)}
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div style={{
                                            ...
                                            background: 'linear-gradient(135deg, rgba(183,110,121,0.30), rgba(212,175,55,0.18))',
                                            border: '1px solid rgba(183,110,121,0.25)',
                                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            fontSize: 15, fontWeight: 600, color: 'rgba(232,220,222,0.9)',
                                            fontFamily: 'var(--font-body)',
                                        }}>
                                            {user?.email?.[0]?.toUpperCase() || '✨'}
                                        </div>
                                        <div>
                                            <p style={{ fontSize: 13, color: 'rgba(232,224,226,0.88)', fontFamily: 'var(--font-body)', fontWeight: 500 }}>
                                                {user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'You'}
                                            </p>
                                            <p style={{ fontSize: 10, color: 'rgba(183,110,121,0.65)', fontFamily: 'var(--font-body)', marginTop: 1 }}>
                                                {isAdmin ? '👑 Admin' : '📖 Diarist'}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <motion.div
                                        ...
                                        style={{
                                            marginTop: 16, height: 1, borderRadius: 1, transformOrigin: 'left',
                                            background: 'linear-gradient(90deg, rgba(183,110,121,0.4) 0%, rgba(212,175,55,0.25) 60%, transparent 100%)',
                                        }}
                                    />
                                </div>

                                {/* Items */}
                                <div className="flex flex-col px-3 py-2 gap-0.5" style={{ position: 'relative', zIndex: 1 }}>
                                    {[
                                        { icon: '🎨', label: 'Aesthetics', desc: 'Fonts, colors & theme', action: () => { setIsMenuOpen(false); setShowCustomization(true) } },
                                        { icon: '📤', label: 'Export Diary', desc: 'Download your entries', action: () => { setIsMenuOpen(false); setShowExport(true) } },
                                        { icon: '🔍', label: 'Advanced Search', desc: 'Filter by mood, date & tags', action: () => { setIsMenuOpen(false); setShowSearch(true) } },
                                    ].map((item, i) => (
                                        <motion.button
                                            key={item.label}
                                            initial={{ x: -20, opacity: 0 }}
                                            animate={{ x: 0, opacity: 1 }}
                                            exit={{ x: -16, opacity: 0, transition: { delay: (2 - i) * 0.035, duration: 0.13 } }}
                                            transition={{ delay: 0.08 + i * 0.055, type: 'spring', stiffness: 300, damping: 26 }}
                                            onClick={item.action}
                                            whileHover={{ x: 3 }}
                                            whileTap={{ scale: 0.97 }}
                                            ...
                                            onMouseEnter={e => {
                                                e.currentTarget.style.background = 'rgba(183,110,121,0.07)'
                                                e.currentTarget.style.borderColor = 'rgba(183,110,121,0.11)'
                                            }}
                                            onMouseLeave={e => {
                                        ...
                                        >
                                            <motion.span
                                                ...
                                                style={{ fontSize: 19, flexShrink: 0, display: 'block', lineHeight: 1 }}
                                            >{item.icon}</motion.span>
                                            <div>
                                                <p style={{ fontSize: 13, color: 'rgba(232,224,226,0.83)', fontFamily: 'var(--font-body)', fontWeight: 500, lineHeight: 1.2 }}>{item.label}</p>
                                                <p style={{ fontSize: 10, color: 'rgba(183,110,121,0.50)', fontFamily: 'var(--font-body)', marginTop: 2 }}>{item.desc}</p>
                                            </div>
                                        </motion.button>
                                    ))}

                                    {/* Divider */}
                                    <div style={{ height: 1, margin: '5px 8px', background: 'rgba(255,255,255,0.04)', borderRadius: 1 }} />
                                    ...
                                    >
                                        <div>
                                            <p style={{ fontSize: 13, color: 'rgba(232,224,226,0.83)', fontFamily: 'var(--font-body)', fontWeight: 500 }}>
                                                {theme === 'dark' ? 'Dark Mode' : 'Light Mode'}
                                            </p>
                                            <p style={{ fontSize: 10, color: 'rgba(183,110,121,0.50)', fontFamily: 'var(--font-body)', marginTop: 2 }}>
                                ...
                                >
                                    <motion.button
                                        ...
                                        whileTap={{ scale: 0.97 }}
                                        className="flex items-center gap-3 text-left w-full mt-3"
                                        style={{
                                            padding: '9px 10px', borderRadius: 11,
                                            color: 'rgba(210,75,75,0.72)', fontFamily: 'var(--font-body)',
                                            background: 'transparent', border: '1px solid transparent',
                                            cursor: 'pointer', transition: 'background 0.15s, border 0.15s, color 0.15s',
                                        }}
                                        onMouseEnter={e => {
                                            e.currentTarget.style.background = 'rgba(210,75,75,0.07)'
                                            e.currentTarget.style.borderColor = 'rgba(210,75,75,0.12)'
                                            e.currentTarget.style.color = 'rgba(210,75,75,0.95)'
                                        }}
                                        onMouseLeave={e => {
                                            e.currentTarget.style.background = 'transparent'
                                            e.currentTarget.style.borderColor = 'transparent'
                                            e.currentTarget.style.color = 'rgba(210,75,75,0.72)'
                                        }}
                                    >
                                        <motion.span
                                            whileHover={{ x: 3 }}
                                            transition={{ type: 'spring', stiffness: 300, damping: 18 }}
                                            style={{ fontSize: 19, display: 'block', lineHeight: 1 }}
                                        >🚪</motion.span>
                                        <div>
                                            <p style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.2 }}>Logout</p>
                                            <p style={{ fontSize: 10, color: 'rgba(210,75,75,0.42)', marginTop: 2 }}>Sign out of Sidekick</p>
                                        </div>
                                    </motion.button>
                ...
                >
                    <div
                        className="relative flex gap-0.5 sm:gap-1 p-1 rounded-2xl"
                        style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.04)', overflowX: 'auto', overflowY: 'visible', scrollbarWidth: 'none' }}
                    >
                        {/* Animated pill */}
                        <motion.div
                        ...
                        {TABS.map((tab) => (
                            <button
                                ...
                                className={`relative z-10 flex-shrink-0 flex-1 min-w-[40px] py-2.5 rounded-xl text-[10px] sm:text-[11px] font-body font-medium transition-colors duration-300
